...
class SingleQubitExperiment(DataOutput):
    ...
    def savePropagator(self, name: str) -> None:
        ...
        np.save(super().createFileName(name + "-" + self.__gate.get_key(), ""), U)

    def plotPropagator(self, name: str) -> None:
        U = self.__experiment.propagators[self.__gate.get_key()]
        utils.plotMatrix(
            U,
            super().createFileName(name + "-" + self.__gate.get_key() + "_a", "png"),
            super().createFileName(name + "-" + self.__gate.get_key() + "_b", "png"),
        )
